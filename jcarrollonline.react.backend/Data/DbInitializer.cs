...
namespace jcarrollonline.react.backend.Data
{
    internal class DbInitializer
    {
        internal static void Initialize(ApplicationDbContext dbContext)
        {
            ...
            //dbContext.Database.ExecuteSqlRaw("DBCC CHECKIDENT('Fora', RESEED, 0)");
            //dbContext.Database.ExecuteSqlRaw("DBCC CHECKIDENT('ForumThread', RESEED, 0)");
            ...
            //    new ForumThread{ Title="C# Reply to first thread", Content="Contents of C# reply to first thread",
            //    Locked=false, CreatedAt=DateTime.UtcNow, UpdatedAt=DateTime.UtcNow,
            //    PostNumber=2, ParentId=1, RootId=1, ForumId=1},
